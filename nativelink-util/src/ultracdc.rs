    ...
    fn decode(&mut self, buf: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        while i < buf.len() {
            ...
            if chunk.len() >= self.min_size {
                ...
                let bytes_mut = BytesMut::from(&chunk[chunk_len - window_size.min(chunk_len)..]);
                let rolling_hash_value = self.rolling_hash(
                    &bytes_mut,
                    window_size,
                ) as u64;

                if (rolling_hash_value & mask) == mask || chunk.len() > self.max_size {
                    split_point = i;
                    break;
                }
            }
